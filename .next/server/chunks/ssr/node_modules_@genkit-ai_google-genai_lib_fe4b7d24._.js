...
__export(converters_exports, {
    ...
    toImagenMedia: ()=>toImagenMedia,
    toImagenPredictRequest: ()=>toImagenPredictRequest,
    toImagenTryOnRequest: ()=>toImagenTryOnRequest,
...
function toImagenMedia(media) {
    if (media.url.startsWith("data:")) {
        ...
        throw new import_genkit.GenkitError({
            status: "INVALID_ARGUMENT",
            message: "Imagen does not support http(s) URIs. Please specify a Cloud Storage URI."
        });
    } else {
        return {
            bytesBase64Encoded: media.url
...
function toImagenTryOnRequest(request) {
    const { ...parameters } = request.config || {};
    const instance = {};
    const personImageMedia = (0, import_utils2.extractMedia)(request, {
    ...
    const productImagesMedia = (0, import_utils.extractMediaArray)(request, {
...
function toImagenPredictRequest(request) {
    const hasTryOnMedia = request.messages.some((m)=>m.content.some((p)=>p.metadata && (p.metadata.type === "personImage" || p.metadata.type === "productImage")));
    if (hasTryOnMedia) {
        return toImagenTryOnRequest(request);
    }
    return {
...
__export(imagen_exports, {
    ImagenConfigSchema: ()=>ImagenConfigSchema,
    ImagenTryOnConfigSchema: ()=>ImagenTryOnConfigSchema,
    KNOWN_MODELS: ()=>KNOWN_MODELS,
    TEST_ONLY: ()=>TEST_ONLY,
    defineModel: ()=>defineModel,
    isImagenModelName: ()=>isImagenModelName,
...
const ImagenTryOnConfigSchema = import_genkit.z.object({
    sampleCount: import_genkit.z.number().int().min(1).optional().describe("Number of images to generate."),
    seed: import_genkit.z.number().int().optional().describe("Random seed for the image generation."),
    baseSteps: import_genkit.z.number().int().min(1).max(100).optional().describe("Number of denoising steps."),
    personGeneration: import_genkit.z.enum([
        "dont_allow",
        "allow_adult",
        "allow_all"
    ...
    storageUri: import_genkit.z.string().optional().describe("Cloud Storage URI to store the generated images."),
    outputOptions: import_genkit.z.object({
        mimeType: import_genkit.z.string().optional(),
        compressionQuality: import_genkit.z.number().int().optional()
    }).optional()
}).passthrough();
function commonRef(name, info, configSchema = ImagenConfigSchema) {
    return (0, import_model.modelRef)({
...
const KNOWN_MODELS = {
    ...
    "virtual-try-on-preview-08-04": commonRef("virtual-try-on-preview-08-04", {
        supports: {
            media: true,
            multiturn: false,
            tools: false,
...
function isImagenModelName(value) {
    return !!value && (value.startsWith("imagen-") || value.startsWith("virtual-try-on-") || value in KNOWN_MODELS);
}
function model(version, config = {}) {
    const name = (0, import_utils.checkModelName)(version);
    if (KNOWN_MODELS[name]) {
